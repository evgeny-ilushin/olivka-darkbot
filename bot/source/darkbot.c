...
bool isRuAlpha2(char c) { return c >= 'ј' && c <= '€'; }
...
char *inv_var_names[MAX_INT_VARS] = {" » ЅјЌЌј’", "Ќ”_„“ќ", "AUTOOP", "AUTOVOICE", "GREETINGS", "WRITEDICT",
                                     "RANDOMSTUFF", "«ј∆ќѕЋ»¬јЌ»≈", "AUTOASS", "VIKTORINA"};
...
void chanserv(char *source, char *target, char *buf),
        ...
        show_url(char *nick, char *topic, char *target, long, long, char *uh,
                 long);
void show_url_2(char *nick, char *topic, char *topic_with_spaces, char *target, long, long, char *uh,
         long);

char *strlwr(char *buf), *rand_reply(const char *nick), *date(),
        *get_multiword_topic(char *first), *revert_topic(char *input),
        *get_multiword_topic_spaced(char *first),
...
void parse_server_msg(fd_set *read_fds, pollfd *pfd);
...
long lcn1 = 0, lcn2 = 0, lcn4 = 0, lcn3 = 0, SeeN = 0, DebuG = 1;
...
struct userlist {                                /* internal userlist */
...
char *
get_word(long number, char *string) {                                /* gets a specific word requested */
    long i = 0;
    char *ptr;
...
*/
time_t return_useridle(const char *chan, const char *who, int toggle) {                                /* toggle=0 is for idle time, toggle=1 is to check if user
	is in the chan */
...
int looks_like_math(char *source, char *text, char *target) {
    int xnc = strlen(text);
    char xvalid[] = "0123456789.-+*^/() ";
        if (text == 0 || xnc < 3) {
            return 0;
        }

        for (int i = 0; i < xnc; i++) {
	if (strchr(xvalid, text[i]) == NULL) {
                return 0;
            }
        }
...
// AutoCalc
        if (looks_like_math(source, bufCopy + 1, target)) {
//	    S("PRIVMSG %s :%s, %s\n", target, source, ),



                snprintf(temp, sizeof(temp),
                         "/home/olivka/bot/calc.sh %s",
                         bufCopy + 1);
...
//S ("PRIVMSG zloy :S: [%s] [%s] [%d] [%s]\n", bufCopy, uu.href, aaa, uu2.href);
                    if (stricmp(uu2.href, "http://temp.2396.ru/") == 0)
                        return;
                    S("PRIVMSG %s :%s, это уже cкурили (%s/%s, %s)\n", target, source, uu2.nick, uu2.chan,
                      nocrlf(ctime(&uu2.date)));
                    db_sleep(5);
                    return;
                } else {
                    uu.nick = source;
    ...
    if (*cmd == *CMDCHAR) {
	...
	else if (stricmp(cmd, "RU") == 0 || stricmp(cmd, "EN") == 0 || stricmp(cmd, "UA") == 0 || stricmp(cmd, "BY") == 0) {
        s2 = bufCopy+4;
            if (s2 == NULL)
                R;
			...
			} else if (stricmp(cmd, "UA") == 0) {
                    	    snprintf(temp, sizeof(temp), "/home/olivka/bot/2ua.sh  %s\n", s2);
			} else {
                    	    snprintf(temp, sizeof(temp), "/home/olivka/bot/2by.sh  %s\n", s2);
			}
                        const char *res = run_program(temp);
        ...
        } else if ((stricmp(cmd, "SEEN") == 0 || stricmp(cmd, "где") == 0) && SeeN == 1) {
            s = strtok(NULL, " ");
            if (s == NULL) {
	  ...
	  * ctcp's */
        ...
        if (s != NULL) {
            ...
            if (stricmp(s, "ADD") == 0
            ...
            } else if (stricmp(s, "ќЅќ—–»") == 0 || stricmp(s, "«ј∆ќѕ№") == 0 ||
                       stricmp(s, "обосри") == 0 || stricmp(s, "зажопь") == 0 ||
                       stricmp(s, "обругай") == 0 || stricmp(s, "ќЅ–”√ј…") == 0) {
                s2 = strtok(NULL, "");
            ...
            } else if (stricmp(s, "–ј— –ј—№") == 0 || stricmp(s, "раскрась") == 0 ||
                       stricmp(s, "ѕќ –ј—№") == 0 || stricmp(s, "покрась") == 0) {
                s2 = strtok(NULL, "");
            ...
            } else if (stricmp(s, "засобачь") == 0 || stricmp(s, "«ј—ќЅј„№") == 0 ||
                       stricmp(s, "засоб@чь") == 0 || stricmp(s, "«ј—ќЅ@„№") == 0) {
                s2 = strtok(NULL, "");
                ...
                S("PRIVMSG %s :%s, %c -> %d\n", target, source, s2[0],
                  s2[0]);
            } else if ((stricmp(s, "SEEN") == 0 || stricmp(s, "где") == 0) && SeeN == 1) {
                s2 = strtok(NULL, " ");
                if (s2 == NULL)
                    R;
...
#endif
            else if (stricmp(s, "Ќ”„“ќ") == 0 || stricmp(s, "празднек") == 0) {
                s2 = strtok(NULL, "");
...
//  урсы говновалют
                ...
                sleep(2);
            } else if (stricmp(s, "рубель") == 0 || stricmp(s, "бульбик") == 0) {
                s2 = strtok(NULL, "");

                snprintf(temp, sizeof(temp), "/home/olivka/bot/byn.sh", s2);
                const char *res = run_program(temp);
                int ll2to = strlen(res);
                if (ll2to > 1) {
                    S("PRIVMSG %s :%s, %s рос≥йських рубл≥в\n", target, source, res);
                ...
                sleep(2);
            } else if (stricmp(s, "фунт") == 0 || stricmp(s, "фунтик") == 0) {
                s2 = strtok(NULL, "");

                snprintf(temp, sizeof(temp), "/home/olivka/bot/pound.sh", s2);
                const char *res = run_program(temp);
                int ll2to = strlen(res);
                if (ll2to > 1) {
                    S("PRIVMSG %s :%s, $%s\n", target, source, res);
...
// Matershinnik N
            else if (stricmp(s, "fucker") == 0 || stricmp(s, "доход€га") == 0 || stricmp(s, "матершинник") == 0 ||
                     stricmp(s, "синеносый") == 0) {
                s2 = strtok(NULL, "");
...
// Game
            else if (stricmp(s, "вращаю") == 0 || stricmp(s, "кручу") == 0 || stricmp(s, "ставлю") == 0 ||
                     stricmp(s, "поиграем") == 0 || stricmp(s, "играю") == 0) {
                s2 = strtok(NULL, "");
                if (s2 != 0) {
                    long sum = atol(s2);
...
//									  if (level < 4 || s2 == NULL || strlen (s2) > 40 || strpbrk(s2, ", \t\r\n|\'\"><&^*?;:"))
                ...
                snprintf(temp, sizeof(temp),
                         "ping %s -c 3 2>&1 | grep \'icmp_seq\\=2\\|ping\\:\\|packet\\ loss\\|bytes\\ of\\ data.\'",
                         s2);
                const char *res = run_program(temp);
                int ll2to = strlen(res);
                if (ll2to > 10) {
...
// DEF codes lookup
            else if (stricmp(s, "phoneid") == 0 || stricmp(s, "2пробей") == 0) {
                s2 = strtok(NULL, "");
...
// hmmm...
                ...
                do {
                    ...
                    *pPort = 0;
                    ...
                    if (strstr(ss2, "192.168.1") || strstr(ss2, "127.0.0.1") || strstr(ss2, "localhost") ||
                        strstr(ss2, "acidflash.org"))
                        break;
                ...
                snprintf(temp, sizeof(temp),
                         "host -W 3 %s | grep \'\\ domain\\ name\\ pointer\\ \\|not\\ found\\|\\ has\\ address\\ \'",
                         ss2);
                const char *res = run_program(temp);
                ...
                if (ll2to > 3) {
                    ...
                    do {
                        ...
                        if (!ll2) {
                            ll2 = strstr((char *) res, (char *) tplx3);
                            tpls = sizeof(tplx3);
                        }
...
// < SET "WHO?" :) 04.2007
                ...
                if (s2 == NULL || strlen(s2) < 1 || strpbrk(s2, ",\t\r\n`\'\"><&^*/;~^"))
                {
                    do_randomtopic(target, DONNO_RDB, source, buf);
                    R;
                }
...
//									  if (level < 4 || s2 == NULL || strlen (s2) > 40 || strpbrk(s2,", \t\r\n|\'\"><&^*"))
                ...
                //snprintf(temp, sizeof(temp), "/home/olivka/bot/scripts/mlookup.sh  %s\n", s2);
                // snprintf (temp,sizeof(temp), "wget 2>&1 --spider --tries=1 --timeout=5 %s | grep \'Length\\:\\|ƒлина\\:\'\n", s2);
                const char *res = run_program(temp);
                int ll2to = strlen(res);
                if (ll2to > 5) {
                    S("PRIVMSG %s :\2%s\2: %s\n", target, source, res);
...
// Num lookup 2019
            else if (stricmp(s, "imsi") == 0 || stricmp(s, "пробей") == 0) {
                s2 = strtok(NULL, "");
...
// <!-- SENDMAIL
                ...
                if (w_now - g_last_sendmail_time <= SENDMAIL_TIME) {
                    S("PRIVMSG %s :%s, please wait \2%d\2s more\n", target, source,
                      SENDMAIL_TIME - (w_now - g_last_sendmail_time) + 1);
                    //do_randomtopic(target, DONNO_RDB, source, buf);
                    R;
                }                                    //
...
// Enable/disable silent channels
            else if (stricmp(s, "смирно") == 0 || stricmp(s, "смирно!") == 0 || stricmp(s, "молчи") == 0 ||
                     stricmp(s, "закройс€") == 0) {
                if (!noChanEnable) {
                    S("PRIVMSG %s :OK, %s :(\n", target, source);
                    noChanEnable = true;
                }
            } else if (stricmp(s, "разрешаю") == 0 || stricmp(s, "вольно") == 0 || stricmp(s, "вольно!") == 0 ||
                       stricmp(s, "говори") == 0) {
                if (noChanEnable) {
                    S("PRIVMSG %s :OK, %s :)\n", target, source);
                    noChanEnable = false;
                ...
                if (unixtime > 86400)
                    ...
                    S
                            ("PRIVMSG %s :%s, %d minute%s, %d sec%s\n",
                             ...
                             unixtime / 60 == 1 ? "" : "s",
                             unixtime % 60, unixtime % 60 == 1 ? "" : "s");
            } else if (stricmp(s, "CPU?") == 0 || stricmp(s, "гор€чий?") == 0) {
                ...
                // <NEW!> 29/03/2006 2:52 am // ѕесдец ночка блиа 8-)

            else if (stricmp(s, "ебут?") == 0 || stricmp(s, "сосуны") == 0 || stricmp(s, "suckers") == 0) {
                do {
                    if (check_access(userhost, target, 0, source) >= ROOT_LEVEL) {
            ...
            } else if (stricmp(s, "красавчег") == 0 || stricmp(s, "ебали?") == 0) {
                do {
                    if (check_access(userhost, target, 0, source) >= ROOT_LEVEL) {
            ...
            } else if (stricmp(s, "моск") == 0 || stricmp(s, "свободно?") == 0 || stricmp(s, "free") == 0) {
                do {
                    //        if (check_access (userhost, target, 0, source) >= ROOT_LEVEL)
                    //	{
            ...
            } else if (stricmp(s, "активной") == 0 || stricmp(s, "влезло?") == 0 || stricmp(s, "т€жко?") == 0) {
                do {
                    //        if (check_access (userhost, target, 0, source) >= ROOT_LEVEL)
                    //	{
            ...
            } else if (stricmp(s, "пассивной") == 0 || stricmp(s, "войдет?") == 0 || stricmp(s, "влезет?") == 0) {
                do {
                    //        if (check_access (userhost, target, 0, source) >= ROOT_LEVEL)
                    //	{
...
*/
                ...
                // URL cache
            else if (stricmp(s, "URL") == 0 || stricmp(s, "бо€н") == 0 || stricmp(s, "бойан") == 0) {
                url_cache
                url4(UCN);
                URL uux;
                if (url4.getLast(&uux)) {
                    S("PRIVMSG %s :%s, \37%s\37  (%s/%s, %s)\n", target, source, uux.href, uux.nick, uux.chan,
                      nocrlf(ctime(&uux.date)));
                } else
                    do_randomtopic(target, DONNO_RDB, source, buf);
                ...
                snprintf(temp, sizeof(temp), "uptime\n");
                S("PRIVMSG %s :ƒела такие, %s: localtime %s\n", target, source, run_program(temp));
                ...
                S("PRIVMSG %s :I am running \2Debian GNU/Linux 4.0\2/%s\n", target,
                  run_program(temp));
            } else if (stricmp(s, "MEM") == 0 || stricmp(s, "зан€то?") == 0 || stricmp(s, "зохавано") == 0) {
                ...
                //strcpy(sx, s);
                show_url_2(source,
                           get_multiword_topic(s), get_multiword_topic_spaced(s), target, 1, 0, userhost, 0);
            }
        } else {
            // S ("PRIVMSG %s :%s\n", target, WHUT);
            // Try to qify? no
...
show_url(char *nick, char *topic,
    ...
    show_url_2(nick, topic, NULL, target, donno, floodpro, uh, no_pipe);
}

void show_url_2(char *nick, char *topic, char *topic_with_spaces,
         char *target, long donno,
         long floodpro, char *uh, long no_pipe) {
    FILE *fp;
...
//					sleep(10000);
    ...
    if (donno == 1) {
        ...
        //s2 = strtok(NULL, "");
        //printf("@@@@@@@@@@@@@@@@@@ S2+: %s", s2);
        if (topic == NULL || topic_with_spaces == NULL) {
            //do_randomtopic(target, WHUT_RDB, source, buf);
            do_randomtopic(target, DONNO_RDB, nick, Data);
        } else {
            char temp1[1024];
            char topic1[1024];
            strcpy(topic1, topic_with_spaces);
            snprintf(temp1, sizeof(temp1),
                     "/home/olivka/bot/scripts/qify.sh %s",topic_with_spaces);
            const char *res = run_program(temp1);
            if (res != NULL && strlen(res) > 2 && strcmp(res, topic1)) {
                S("PRIVMSG %s :\2%s\2, %s\n", target, nick, res);
            } else
                //do_randomtopic(target, WHUT_RDB, source, buf);
                do_randomtopic(target, DONNO_RDB, nick, Data);
...
//						  S("MODE %s +hoa %s %s %s\n", s1, s, s, s);
                    ...
                    if (aLevel >= 4) {
                        if (inv_vars[INTV_AOP] && inv_vars[INTV_AVOICE])
                            ...
                            S("MODE %s +v %s\n", s1, s);
                    } else if (aLevel >= 1 && inv_vars[INTV_AVOICE])
                        S("MODE %s +v %s\n", s1, s);
#if HELP_GREET == 1
                                                                                                                                            }
...
long
setinfo_lastcomm(char *rest) {                               /* Disallows joins to more than one channel (or the same chan)
...
int match_wild1(const char *pattern, const char *str) {
    //char p[10240];
    //char s[10240];
    //strcpy(p, pattern);
    //strcpy(s, str);
    int res = match_wild(pattern, str);
    //printf("MW (%s, %s) -> %n\n", p, s, res);
    return res;
}
...
parse_server_msg(fd_set *read_fds, pollfd *pfd_read) {
...
/*
parse_server_msg(fd_set *read_fds, pollfd *pfd) {
...
void addNick(char *buffer, int bufLength, int nType) {
    if ((nType == MALE) && (num_male_nicks < MAX_NICK - 1) && (bufLength > 0)) {
        ...
        num_male_nicks++;
    } else if ((nType == FEMALE) && (num_female_nicks < MAX_NICK - 1) && (bufLength > 0)) {
    ...
    if (!strcmp(pCommand, "dateinfo") || !strcmp(pCommand, "dateinfo2")) {
        ...
        if (numItems) {
            if (numItems > 1) {
                ...
                sprintf(line, "—егодн€ \2%s", diSet[0].text);
                S("PRIVMSG %s :%s\n", target, line);
            }
        } else
            do_randomtopic(target, "dateprefix3", nick, topic);
    ...
    if (!strcmp(pCommand, "dateinfoex")) {
        ...
        if (numItems > 0) {
            if (numItems > 1) {
                ...
                for (i = 0; i < numItems; i++) {
                    sprintf(line, "%d. [%d] %s (\2%s\2, %s)", i + 1, diSet[i].id, diSet[i].text, diSet[i].nick,
                            nocrlf(ctime(&diSet[i].created)));
                    S("PRIVMSG %s :%s\n", target, line);
                }
            } else {
                do_randomtopic(target, "dateprefix2", nick, topic);

                sprintf(line, "[%05d] —егодн€ \2%s (\2%s\2, %s)", diSet[0].id, diSet[0].text, diSet[0].nick,
                        nocrlf(ctime(&diSet[0].created)));
                S("PRIVMSG %s :%s\n", target, line);
            }
        } else
            do_randomtopic(target, "dateprefix3", nick, topic);
...
*/
    if (!strcmp(pCommand, "bci")) {
        ...
        bc_info5(RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bci10")) {
        char RRR[4096];
        bc1_info10(RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bcf")) {
        char RRR[4096];
        bc_info5_f(RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bcuser")) {
        char RRR[4096];
        bc_infoU(topic, RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bci1")) {
        char RRR[4096];
        bc1_info5(RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bci101")) {
        char RRR[4096];
        bc_info10(RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bcf1")) {
        char RRR[4096];
        bc1_info5_f(RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else if (!strcmp(pCommand, "bcuser1")) {
        char RRR[4096];
        bc1_infoU(topic, RRR);
        S("PRIVMSG %s :%s, %s\n", target, nick, RRR);
    } else
...
void showDateDirect(char *channel, char *nick, time_t time) {
    ...
    if (numItems) {
        if (numItems > 1 && numItems < 100) {
            for (i = 0; i < numItems; i++) {
                sprintf(line, "%d. [\2%d\2] %s  (\2%s\2, %s)", i + 1, diSet[i].id, diSet[i].text, diSet[i].nick,
                        nocrlf(ctime(&diSet[i].created)));
                S("PRIVMSG %s :%s\n", channel, line);
            }
        } else {
            S("PRIVMSG %s :%s, [%d] %s  (\2%s\2, %s)\n", channel, nick, diSet[0].id, diSet[0].text, diSet[0].nick,
              nocrlf(ctime(&diSet[0].created)));
        }
    } else
...
void case2(char *text, int mode) {
    char lChr[] = "abcdefghijklmnopqrstuvwxyzабвгдеЄжзийклмнопрстуфхцчшщьыъэю€";
    char uChr[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZјЅ¬√ƒ≈®∆«»… ЋћЌќѕ–—“”‘’÷„Ўў№џЏЁёя";
...
void setIntVar(char *channel, char *nick, char *variable, char *value) {
    ...
    } else if (!strcmp(pUcValue, "OFF") || !strcmp(pUcValue, "FALSE")) {
        if (inv_vars[varId] > 0) {
            inv_vars[varId] = 0;
...
int t_topegInfo(char *channel, char *nick) {
    ...
    S("PRIVMSG %s :%s, балтека: %s, клерки: %s, рабы: %s\n", channel, nick, ts[0], ts[1], ts[2]);
    return 0;
}
...
int t_glotokInfoUa(char *channel, char *nick) {
    ...
    for (int i = 18; i <= 20; i++) {
        ...
        if (hDiff > 0) {
            sprintf(ts[i - 18], "\2%d %s %d %s\2", hDiff,
...
void dictLatin(char *nick, char *chan, char *src) {
        ...
        // RUS -> LAT
    {
        ...
        char buf[1024] = "";
        _dictfile
        f("/home/olivka/bot/dict/rus2lat.dict2");
        ...
        // LAT -> RUS
    {
        ...
        char buf[1024] = "";
        _dictfile2
        f("/home/olivka/bot/dict/lat2rus.dict3");
